...
describe("LocalPolicy API", function () {
  describe("Tests with basic user", function() {
    let local_policy;
    ...
    it("it should not add a local policy on /local_policy POST", async () => {
      let response = await chai
      .request(server)
      .post("/api/local_policy/")
      .set("Authorization", "Bearer " + tokens)
      .send({
        title: "Test Title",
      ...
      expect(response.text).to.equal("You don't have enough permission to perform this action");
  
      new_local_policy = response.body;
    });
  });

  describe("Tests with ephi user", function() {
    let local_policy;
    let new_local_policy;
    let user;
    var tokens;
