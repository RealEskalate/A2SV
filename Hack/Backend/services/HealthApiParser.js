...
exports.getHealthStatistics = (req, res, respond, rates = false) => {
  if (req.query.country.toLowerCase() === "world") {
    ...
    let request_url = `https://api.covid19api.com/total/country/${req.query.country.toLowerCase()}${set_start_date_for_countries(req).slice(6)}${set_end_date_for_countries(req).slice(4)}`;
    do_api_call(request_url, req.query.criteria, false, res, respond, rates);
  }
