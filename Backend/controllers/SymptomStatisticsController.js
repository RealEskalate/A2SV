...
exports.get_symptom_logs = async (req, res) => {
    ...
    if (language) {
        for (var index = 0; index < logs.length; index++) {
            ...
            if (genderNames) {
                logs[index].user_id.gender =
                    genderNames[logs[index].user_id.gender];
            }
        }
    }
    // translation end
...
exports.get_logs_by_user_id = async (req, res) => {
    ...
    if (!log) {
        ...
        try {
            ...
            log.current_symptoms.risk_score = risk;
            log.current_symptoms.risk_score_level = risk_lvl;
            res.send(log);
        } catch (err) {
            res.status(500).send(err.toString());
        }
