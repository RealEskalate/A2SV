...
exports.get_statistics = async (req, res) => {
  ...
  if (
    ["Confirmed", "Recovered", "Deaths", "Active", "Tests", "All"].includes(
      req.query.criteria
    )
  ) {
    ...
    ["Tests_Rate", "Recovered_Rate", "Deaths_Rate", "Active_Rate"].includes(
      req.query.criteria
    )
  ) {
    if (req.query.country == "World") {
...
exports.getWorldStats = async (req, res) => {
  ...
  let rates = false;
  if (
    ["Test_Rate", "Recovered_Rate", "Deaths_Rate", "Active_Rate"].includes(
      req.query.criteria
    )
  ) {
    req.query.criteria = req.query.criteria.split("_")[0];
