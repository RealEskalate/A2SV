...
exports.get_new_citizen_symptoms_updates = async () => {
    ...
    const today_db = new Date(
        Date.parse(new Date().toISOString().slice(0, 10) + "T21:00:00.000Z")
    );
    ...
    //If existing, modify and save. If not add a new entry to the db
    const check = await NewCitizenSymptoms.findOne({
        date: today_db,
    });
    if (check) {
        check.total = users;
        ...
        const newDayEntry = new NewCitizenSymptoms({
            date: today_db,
            total: users,
        });
        await newDayEntry.save();
...
exports.update_ethiopian_statistics = async () => {
    ...
    if (data) {
        ...
        for (var index = 0; index < data.statewise.length; index++) {
            ...
            let state =
                case_data.state == "Total" ? "Ethiopia" : case_data.state;
            let region_code =
    ...
    for (let ind = 0; ind < 3; ind++) {
        ...
        for (let i = 0; i < length; i++) {
            ...
            if (!map_datas[`${data["Country/Region"]}`]) {
                ...
                try {
                    let current_data =
                        map_datas[`${data["Country/Region"]}`].Data;
                    date_keys.forEach((date) => {
                        if (
                            current_data[`${date}`] &&
